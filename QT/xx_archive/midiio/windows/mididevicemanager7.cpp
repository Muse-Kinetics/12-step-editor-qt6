...
void MidiDeviceManager::slotRestoreGlobals(QByteArray globalsByteArray)
{
    ...
    qDebug() << "RESTORE globals size" << globalsByteArray.size();
...
{
    qDebug() << "Process SysEx" << sysExMessageByteArray.size();
    ...
    else if(sysExDecode->slotIsGlobals(sysExMessageByteArray))
    {
        if(fwUpdateRequested)
        {
            ...
            qDebug() << "_______________________ GLobals Stored +++++++++++++++++++" << globals.size();
...
void MidiDeviceManager::slotRequestFirmwareUpdate()
{
    ...
    if(slotGetMidiOutDevices().contains("QuNexus"))
    {
...
bool MidiDeviceManager::slotConnectDisconnectDevice()
{
    ...
    if(slotGetMidiInDevices().contains("QuNexus"))
    {
        slotCloseMidiIn();
        slotCloseMidiOut();

        slotOpenMidiIn(slotGetMidiInDevices().value("QuNexus"));
        slotOpenMidiOut(slotGetMidiOutDevices().value("QuNexus"));
        emit signalQuNexusConnected(true);
        connected = true;
        qDebug() << "___ Connected ___";
        slotQueryQuNexus();
    }
    else if(!slotGetMidiInDevices().contains("QuNexus"))
    {
        qDebug() << "close????";
        slotCloseMidiIn();
        slotCloseMidiOut();
